...
const InventoryManagement = () => {
  ...
  const handleAddItem = async (newItem) => {
    try {
      await axios.post("/items/create", newItem);
      await fetchItems();
  ...
  const handleUpdateItem = async (updatedItem) => {
    try {
      await axios.put(`/items/update/${selectedItem._id}`, updatedItem);
      await fetchItems();
  ...
  const handleDeleteItem = async () => {
    try {
      await axios.delete(`/items/delete/${selectedItem._id}`);
      await fetchItems();
  ...
  const handleStockAdjustment = async (amount) => {
    try {
      ...
      await axios.put(`/items/${endpoint}/${selectedItem._id}`, { amount });
      await fetchItems();
